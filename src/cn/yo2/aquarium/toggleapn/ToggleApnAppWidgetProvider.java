...
public class ToggleApnAppWidgetProvider extends AppWidgetProvider {
	...
	private static final int BUTTON_LIGHT = 1;

	@Override
	public void onEnabled(Context context) {
		PackageManager pm = context.getPackageManager();
		pm.setComponentEnabledSetting(new ComponentName(
				"cn.yo2.aquarium.toggleapn", ".ToggleApnAppWidgetProvider"),
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (ConnectivityManager.CONNECTIVITY_ACTION.equals(intent.getAction())) {
			...
			} else if (buttonId == BUTTON_LIGHT) {
//				sLightState.toggleState(context);
				Intent intent2 = new Intent(context, WhiteScreenActivity.class);
	...
	public static void updateWidget(Context context) {
		...
		gm.updateAppWidget(new ComponentName(context.getApplicationContext(),
				ToggleApnAppWidgetProvider.class), views);
	}
	...
	static RemoteViews buildUpdate(Context context, int appWidgetId) {
		...
		views.setOnClickPendingIntent(R.id.btn_data,
				getLaunchPendingIntent(context, appWidgetId, BUTTON_DATA));
		views.setOnClickPendingIntent(R.id.btn_light,
				getLaunchPendingIntent(context, appWidgetId, BUTTON_LIGHT));
	...
	private static final StateTracker sLightState = new LightStateTracker();

	/**
	 * The state machine for Data, Wifi and Bluetooth toggling, tracking reality
	 * versus the user's intent. This is necessary because reality moves
	 * relatively slowly (turning on &amp; off radio drivers), compared to
	 * user's expectations.
	 */
	...
	private static final class LightStateTracker extends StateTracker {
		...
		public int getActualState(Context context) {
			return FlashLightHelper.isFlashLightOn() ? STATE_ENABLED : STATE_DISABLED;
		}

		@Override
		protected void requestStateChange(Context context, boolean desiredState) {
			FlashLightHelper.toggleFlashLight(desiredState);
		}
	...
	private static final class MobileDataStateTracker extends StateTracker {
		...
		protected void requestStateChange(Context context, boolean desiredState) {
			...
			if (connectivityManager != null) {
				ConnectivityHelper.setMobileDataEnabled(connectivityManager,
						desiredState);
			}
		}
	...
	private static void updateButtons(RemoteViews views, Context context) {
		switch (sLightState.getTriState(context)) {
		case STATE_DISABLED:
			views.setImageViewResource(R.id.img_light,
					R.drawable.ic_appwidget_settings_brightness_off);
			views.setImageViewResource(R.id.ind_light,
